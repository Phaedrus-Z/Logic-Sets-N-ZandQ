...
\begin{document}
	...
	\chapter{Set Theory}
		...
		\section{Images and inverse images}
			In the previous section, we defined functions. For any function $f:X\to Y$, it can take individual element $x\in X$ to a unique element $y\in Y$. Similarly, $f$ can also take a subset of $X$ to a unique subset in $Y$:
			...
			\begin{remark}~
				\begin{itemize}
					\item The set $f(S)$ is well defined due to the Axiom of Replacement.
					\item $f(S)$ can also be well defined by the Axiom of Seperation and the definition goes like this:\\
					...
					and $f(S)$ is called the (forward) image of $S$ under the map $f$.
					\item To compute $f(S)$, we take every element $x$ of $S$ and apply $f$ to each $x$ individualy, then put all the resulting objects together to form a new set.
				\end{itemize}
			\end{remark}
			However, we should aware that $f(S)$ may sometimes be smaller than $S$ if $f$ is not an injection:
			\begin{counterexample}[$f(S)$ can be smaller than $S$]
				Let $f:\mathbb{Z}\to\mathbb{Z}$ given by $x\mapsto x^2$, then $f(\{-1,0,1\})=\{0,1\}$.
			\end{counterexample}
