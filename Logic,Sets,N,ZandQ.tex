		...
		\section{Functions}
			...
			\begin{proof}~
				\begin{enumerate}[label=($\alph*$)]
					...
					\end{itemize}
					We finally demonstrate that $h$ is unique. Suppose for the sake of contradicition that $\exists h':X\cup Y\to Z$ such that $h'\circ\iota_{X\to X\cup Y}=f$ and $h'\circ\iota_{Y\to X\cup Y}=g$. Then $\forall x\in X\cup Y$:
					(Under construction.)
				\end{enumerate}
			\end{proof}
