...
\newtheorem{counterexample}{\protect\textdbend Counterexample}[section]
...
\begin{document}
	...
	\chapter{Set Theory}
		...
		\section{Functions}
			...
			\end{lemma}
			\begin{proof}Since $f$ is a bijection and $f^{-1}$ is its inverse, $\forall x\in X\implies\exists!y\in Y[y=f(x)\land x=f^{-1}(y)]$. Then:
				\begin{itemize}
					\item  $\forall x\in X$: $f^{-1}[f(x)]=f^{-1}(y)=x$
					Thus, $\forall x\in X~f^{-1}[f(x)]=x$.
					\item $\forall y\mspace{1mu}\in Y\mspace{1mu}$: $\mspace{1mu}f[f^{-1}(y)]=\mspace{9mu}f(x)\mspace{10mu}=y\mspace{1mu}$
					Thus, $\forall y\mspace{1mu}\in Y\mspace{1mu}~f[f^{-1}(y\mspace{1mu})]=y$.
				\end{itemize}
				...
				If $f^{-1}$ is not injective, then let $y_1:=f(x_1)$ and $y_2:=f(x_2)$ we have:
				$$\exists y_1\!\neq\!y_2\!\in\! Y\,f^{-\!1}\!(y_1)\!=\!f^{-\!1}\!(y_2)\!\implies\!\exists y_1\!\neq\!y_2\!\in\!Y\,x_1\!=\!x_2\!\implies\!\exists y_1\!\neq\!y_2\!\in\!Y\,f(x_1)\!=\!f(x_2)\!\implies\!\exists y_1\!\neq\!y_2\!\in\!Y\,y_1\!=\!y_2\!$$
				a contradiction; $f$ must be injective.\\
				If $f^{-1}$ is not surjective, then since $f$ is surjective:
				$$\exists x_0\in X~\forall y\in Y~f^{-1}(y)\neq x_0\implies\exists x_0\in X~\forall x\in X~x\neq x_0,$$
				ridiculous; $f$ must also be surjective.\\
				In all, $f$ must be both injective and surjective and therfore be invertible.
			\end{proof}
			\begin{lemma}
